...
async def get_all_companies(
    ...
    for c in companies:
        ...
        result.append(CompanyWithModules(
            id=c.id, name=c.name, slug=c.slug, email_domain=c.email_domain,
            primary_color=c.primary_color, secondary_color=c.secondary_color,
            description=c.description, logo_url=c.logo_url,
            is_active=c.is_active, created_at=c.created_at, modules=modules,
        ))
    return result
...
async def get_my_company(
    ...
    return CompanyWithModules(
        id=company.id, name=company.name, slug=company.slug, email_domain=company.email_domain,
...
async def create_company(
    ...
    company = Company(
        name=data.name, slug=data.slug, logo_url=data.logo_url, email_domain=data.email_domain,
...
async def update_company_modules(
    ...
    for mod in data.modules:
        if mod in AVAILABLE_MODULES:
            db.add(CompanyModule(company_id=company_id, module_name=mod, is_enabled=True))
    db.commit()
    modules = [m.module_name for m in db.query(CompanyModule).filter(
